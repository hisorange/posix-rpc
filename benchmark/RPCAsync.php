...
if ($is_master) {
    ...
    if ($child_idx !== 0) {
        $ch = 'hash.' . $child_idx;
        $node->respond($ch, function (Message $msg) {
    ...
    while (true) {
        ...
        if ($child_idx === 0) {
            for ($i=0; $i < 5; $i++) {
                $result = $node->request('hash.' . $i, ($messages . 'x' . $i), function (IMessage $response) use (&$messages, &$logger) {
                    $messages++;

                    if ($messages % 100 === 0) {
                        $logger->warning('Got the response [' . $response->getPayload() . '] total message [' . $messages . '] count');
                    }
                });
            }
        }
        ...
        if (time() >= BENCH_ENDS_AT) {
            $logger->info("Reached the end time FINAL: " . $messages);
            exit(0);
        }
